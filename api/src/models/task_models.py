...
class TaskBase(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=1000, description="Detailed task description (optional, up to 1000 characters)")
    priority: Priority = Field(Priority.medium, description="Task priority level")
    category: Optional[str] = Field(None, max_length=50, description="Task category or tag (optional, up to 50 characters)")
    due_date: Optional[date] = Field(None, description="Due date in YYYY-MM-DD format (optional)")
...
class TaskResponse(TaskBase):
    id: str = Field(..., description="Unique task identifier (e.g., UUID or generated key)")
    status: TaskStatus = Field(TaskStatus.pending, description="Current task status")
    created_at: datetime = Field(..., description="Timestamp when the task was created (Unix timestamp or ISO format)")
    updated_at: datetime = Field(..., description="Timestamp of the last update (Unix timestamp or ISO format)")
    completed_at: Optional[datetime] = Field(None, description="Timestamp when the task was completed (null if not completed)")
